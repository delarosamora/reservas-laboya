          ...
          <ul class="dropdown-menu dropdown-menu-end">
            ...
            <li>
              <a class="dropdown-item {{ request()->routeIs('settings.password') ? 'active' : '' }}" href="{{ route('settings.password') }}" wire:navigate>
                <i class="icon-base bx bx-cog icon-md me-3"></i><span>{{ __('Settings') }}</span>
              </a>
            </li>
            <li>
              <div class="dropdown-divider my-1"></div>
            ...
            <li>
              <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button class="dropdown-item" type="submit" class="btn p-0"><i class="icon-base bx bx-power-off icon-md me-3"></i><span>{{ __('Log Out') }}</span></button>
              </form>
            </li>
          </ul>
        @else
