<form wire:submit="save">
    <div class="row mb-6">
      <div class="col">
        <label class="form-label" for="member" id="member-label">{{ __('Member') }}</label>
        <div class="input-group input-group-merge">
            <span class="input-group-text"><i class="icon-base bx bx-user"></i></span>
            <x-forms.select-input id="member" name="member_id" wire:model.change="member_id">
                <option value="" selected>{{ __('Select a member') }}</option>
                @foreach ($this->members as $member)
                  <option value="{{ $member->id }}">{{ $member->number }} {{ $member->full_name }}</option>
                @endforeach
